...
@Model.register("biaffine_parser_chinese")
class BiaffineChineseDependencyParser(Model):
    ...
    def __init__(self,
        ...
        super(BiaffineChineseDependencyParser, self).__init__(vocab, regularizer)
        ...
        self._endpoint_span_extractor = EndpointSpanExtractor(self.text_field_embedder.get_output_dim(),
                                                              combination="x,y",
                                                              bucket_widths=False)
    ...
    def forward(self,  # type: ignore
                characters: Dict[str, torch.LongTensor],
                character_spans: torch.IntTensor, ## Shape: batch x num_spans x 2
                pos_tags: torch.LongTensor,
                metadata: List[Dict[str, Any]],
                head_tags: torch.LongTensor = None,
                head_indices: torch.LongTensor = None) -> Dict[str, torch.Tensor]:

        embedded_text_input = self.text_field_embedder(characters)
        character_mask = util.get_text_field_mask(characters).float()
        ...
        endpoint_span_embeddings = self._endpoint_span_extractor(embedded_text_input, spans)
        # Shape: (batch_size, num_spans, emebedding_size)
        attended_span_embeddings = self._attentive_span_extractor(embedded_text_input, spans)

        # Shape: (batch_size, num_spans, emebedding_size + 2 * encoding_dim + feature_size)
        embedded_text_input = torch.cat([endpoint_span_embeddings, attended_span_embeddings], -1)
        ...
        predicted_heads, predicted_head_tags, mask, arc_nll, tag_nll = self._parse(
            embedded_text_input, span_mask, head_tags, head_indices)
